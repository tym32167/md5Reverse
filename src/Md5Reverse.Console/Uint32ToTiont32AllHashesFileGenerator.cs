...
namespace Md5Reverse.Console
{
    public class Uint32ToTiont32AllHashesFileGenerator
    {
        ...
        public void GenerateFile(string folder, IProgress<int> progress)
        {
            var step1Folder = Path.Combine(folder, "step1");
            Step1_Generate256UnsortedFiles(step1Folder, progress);

            Step2_GenerateIndexAndIdsFile(step1Folder, folder, progress);
            Directory.Delete(step1Folder, true);
        }

        private void Step1_Generate256UnsortedFiles(string folder, IProgress<int> progress)
        {
        ...
        private void Step2_GenerateIndexAndIdsFile(string step1Folder, string step5Folder, IProgress<int> progress)
        {
            if (!Directory.Exists(step5Folder))
                Directory.CreateDirectory(step5Folder);

            // Timing: 0:15:33
            using (_log.Timing(nameof(Step2_GenerateIndexAndIdsFile)))
            {
